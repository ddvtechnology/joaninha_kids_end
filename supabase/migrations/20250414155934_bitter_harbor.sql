/*
  ...
  2. Remove any auth customizations
  3. Keep only essential business tables
  4. Use simple text fields for user references
*/
...
-- Drop any existing auth related functions/triggers
...
CREATE POLICY "Enable write for authenticated users" ON products FOR ALL TO authenticated WITH CHECK (true);

CREATE POLICY "Enable read for authenticated users" ON customers FOR SELECT TO authenticated USING (true);
CREATE POLICY "Enable write for authenticated users" ON customers FOR ALL TO authenticated WITH CHECK (true);
...
CREATE POLICY "Enable write for authenticated users" ON expenses FOR ALL TO authenticated WITH CHECK (true);
